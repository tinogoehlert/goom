...
//**	   #    ##    ##    #      ####     ####   ##       ##
//**
//**	$Id: level.cpp 1625 2006-07-30 16:35:14Z dj_jl $
//**
...
static void *GLVisMalloc(size_t size)
{
	...
	if (!ptr)
	{
		throw GLVisError("Couldn't alloc %lu bytes", size);
	}
	memset(ptr, 0, size);
	return ptr;
}
...
void TVisBuilder::Run(uint8_t *wad, uint8_t *gwa)
{
...
TOWadFile* TVisBuilder::GetOutput() 
{
	return &this->outwad;
}
...
void TGLVis::Build(uint8_t *wad, uint8_t *gwa)
{
	try
	{
		TVisBuilder VisBuilder(*this);
		VisBuilder.Run(wad,gwa);
	}
	catch (WadLibError &e)
	{
